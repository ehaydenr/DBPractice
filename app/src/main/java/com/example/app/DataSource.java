...
public class DataSource {
    ...
    public ArrayList<Recipe> searchRecipesIngredients(String[] keyword){
        ArrayList<Recipe> recipes = new ArrayList<Recipe>();
        String queryText = "";
        for(int i = 0; i<keyword.length - 1; i ++){
            queryText = queryText + "'%" + keyword[i] + "%'" + " OR ";
        }
        queryText = queryText + "'%" + keyword[keyword.length-1] + "%'";
    ...
    public int score(String[] keyword, Recipe r){
        int score = 0;
        ...
        for(int i = 0; i < keyword.length; i++){
            for(int j = 0; i < split.length; i++){
                if(keyword[i].equals(split[j])) score ++;
            }
        }
        return score;
    }

    public void sortBasedOnScore(String[] keyword, ArrayList<Recipe> recipes){
